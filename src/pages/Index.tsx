...
const Index = () => {
  ...
  // ⬇ ONLY USING STATIC THEMES NOW
  const [themes, setThemes] = useState<any[]>([]);

  const [featuredProducts, setFeaturedProducts] = useState<any[]>([]);
  const { addToCart } = useCart();

  // ❌ REMOVED: getThemes() from supabase  
  // ✔ Instead, load first 3 static themes
  ...
  useEffect(() => {
    // ✔ Load FIRST 3 themes from themes.ts
    setThemes(staticThemes.slice(0, 3));
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex space-x-4">
                <Button variant="ghost" size="sm"><Instagram className="h-5 w-5" /></Button>
                <Button variant="ghost" size="sm"><Twitter className="h-5 w-5" /></Button>
                <Button variant="ghost" size="sm"><Facebook className="h-5 w-5" /></Button>
              </div>
            </div>
