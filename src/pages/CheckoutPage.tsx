...
import { MapPin, Truck, CreditCard, Wallet, Banknote, Save, Loader2, Crosshair } from 'lucide-react';
...
const CheckoutPage = () => {
  ...
  // Generic Fallback (New Delhi) if geolocation is denied
  const FALLBACK_CENTER = { lat: 28.6139, lng: 77.2090 };

  // Initialize Data & Location
  useEffect(() => {
    const initData = async () => {
      // FIX: Changed from getSession() to getUser()
      ...
      // 1. Try to fetch saved address from DB
      ...
      if (address) {
        ...
        // 2. If no address, start in Edit Mode and Detect Location
      ...
      if (navigator.geolocation) {
          ...
          navigator.geolocation.getCurrentPosition(
              (position) => {
                  ...
                  map?.panTo(pos);
                  map?.setZoom(17);
                  fetchAddressFromCoordinates(pos.lat, pos.lng); // Auto-fill form
                  setMapLoading(false);
              },
              () => {
                  // Error/Denial
  ...
  const onLoadMap = useCallback((mapInstance: google.maps.Map) => {
      setMap(mapInstance);
      // If we have a marker position already (from initData), pan to it
      if (markerPosition) {
  ...
  const initializeRazorpay = async () => {
      ...
      const options = {
          key: "rzp_test_Ri5jk85JFwqzoQ", amount: Math.round(total * 100), currency: "INR", name: "Campus Bazaar",
          handler: function (response: any) { toast.success(`Payment Successful!`); finalizeOrder('paid', response.razorpay_payment_id); },
          prefill: { name: user?.user_metadata?.full_name, email: user?.email, contact: addressForm.phone || '9999999999' },
          theme: { color: "#3399cc" }
      };
  ...
  const finalizeOrder = async (paymentStatus: string, paymentId?: string) => {
    try {
        const orderDetails = cart.map(item => ({ product_id: item.product_id, quantity: item.quantity, price: item.products?.price, name: item.products?.name }));
        const { data: orderData, error } = await supabase.from('orders').insert({
            user_id: user.id, total_amount: total, status: 'placed', order_type: orderType, payment_method: paymentMethod, payment_status: paymentStatus, order_details: orderDetails
        }).select().single();
        if (error) throw error;
        await clearCart();
                ...
                <CardContent>
                    <RadioGroup value={orderType} onValueChange={(v) => setOrderType(v as any)} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className={`flex items-center space-x-2 border p-4 rounded-lg cursor-pointer ${orderType === 'delivery' ? 'border-primary bg-primary/5' : ''}`}>
                            <RadioGroupItem value="delivery" id="delivery" /><Label htmlFor="delivery" className="flex-grow cursor-pointer flex items-center gap-3"><Truck className="h-5 w-5" /> Home Delivery</Label>
                        </div>
                        <div className={`flex items-center space-x-2 border p-4 rounded-lg cursor-pointer ${orderType === 'pickup' ? 'border-primary bg-primary/5' : ''}`}>
                            <RadioGroupItem value="pickup" id="pickup" /><Label htmlFor="pickup" className="flex-grow cursor-pointer flex items-center gap-3"><MapPin className="h-5 w-5" /> Store Pickup</Label>
                        </div>
                    </RadioGroup>
                </CardContent>
            </Card>
            ...
            {orderType === 'delivery' && (
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        {!isEditingAddress && savedAddress && <Button variant="ghost" size="sm" onClick={() => setIsEditingAddress(true)}>Change</Button>}
                    </CardHeader>
                    <CardContent>
                        {isEditingAddress ? (
                                        ...
                                        <LoadScript googleMapsApiKey={GOOGLE_MAPS_API_KEY} libraries={LIBRARIES}>
                                            <div className="absolute top-2 left-2 right-2 z-10 flex gap-2">
                                                <Autocomplete onLoad={onLoadAutocomplete} onPlaceChanged={onPlaceChanged} className='flex-grow'>
                                                    <Input placeholder="Search your location..." className="bg-background shadow-md w-full" />
                                                </Autocomplete>
                                            ...
                                            >
                                                {markerPosition && <Marker position={markerPosition} draggable={true} onDragEnd={onMarkerDragEnd} animation={window.google ? window.google.maps.Animation.DROP : undefined} />}
                                            </GoogleMap>
                                        </LoadScript>
                                    ) : <div className="h-full flex items-center justify-center bg-muted text-muted-foreground">API Key Missing</div>}
                                </div>
                ...
                <CardContent>
                    <RadioGroup value={paymentMethod} onValueChange={(v) => setPaymentMethod(v as any)} className="space-y-3">
                        <div className={`flex items-center space-x-3 border p-4 rounded-lg cursor-pointer ${paymentMethod === 'card' ? 'border-primary ring-1 ring-primary' : ''}`}>
                            <RadioGroupItem value="card" id="card" /><Label htmlFor="card" className="flex-grow cursor-pointer flex items-center gap-3"><CreditCard className="h-5 w-5 text-blue-600" /> Card (Razorpay)</Label>
                        </div>
                        <div className={`flex items-center space-x-3 border p-4 rounded-lg cursor-pointer ${paymentMethod === 'upi' ? 'border-primary ring-1 ring-primary' : ''}`}>
                            <RadioGroupItem value="upi" id="upi" /><Label htmlFor="upi" className="flex-grow cursor-pointer flex items-center gap-3"><Wallet className="h-5 w-5 text-green-600" /> UPI (Razorpay)</Label>
                        </div>
                        <div className={`flex items-center space-x-3 border p-4 rounded-lg cursor-pointer ${paymentMethod === 'cod' ? 'border-primary ring-1 ring-primary' : ''}`}>
                            <RadioGroupItem value="cod" id="cod" /><Label htmlFor="cod" className="flex-grow cursor-pointer flex items-center gap-3"><Banknote className="h-5 w-5 text-orange-600" /> Cash on Delivery</Label>
                        </div>
                    </RadioGroup>
                </CardContent>
            </Card>
            ...
            <Card className="sticky top-24">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-4 flex items-start space-x-2">
                    <Checkbox id="terms" checked={termsAccepted} onCheckedChange={(c) => setTermsAccepted(c as boolean)} /><Label htmlFor="terms" className="text-sm text-muted-foreground">I agree to Terms & Conditions</Label>
                </div>
              </CardContent>
              <CardFooter>
                <Button className="w-full py-6 text-lg" onClick={handlePlaceOrder} disabled={loading || cart.length === 0}>{loading ? 'Processing...' : `Pay ₹${total.toFixed(2)}`}</Button>
              </CardFooter>
            </Card>
          </div>
        </div>
